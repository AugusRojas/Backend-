...
const leerMozosSector = (req, res) => {
  conexion.query(
    "SELECT mozos.nombreMozo, mozos.apellidoMozo, sectores.nombreSector FROM mozos JOIN sectores ON mozos.idSector = sectores.idSector;",
    (err, results) => {
      if (err) {
...
const leerSectores = (req, res) => {
  conexion.query("SELECT * FROM sectores", (err, results) => {
    if (err) {
